...
class MultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        q_proj = self.query_proj(query)  # [bs, query_len, num_heads * head_dim_q]
        k_proj = self.key_proj(key)  # [bs, seq_len, num_heads * head_dim_q]
        v_proj = self.value_proj(value)  # [bs, seq_len, num_heads * head_dim_v]

        # Reshape to [bs, num_heads, seq_len, head_dim]
        # For keys, we transpose after splitting to get shape [bs, num_heads, head_dim, seq_len]
        ...
        # Reshape from [bs, num_heads, query_len, head_dim_v] to [bs, query_len, num_heads * head_dim_v]
        output = (
