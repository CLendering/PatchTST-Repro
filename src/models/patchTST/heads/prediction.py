        ...
        if self.individual:
            ...
            for var_idx in range(self.num_vars):
                z = self.flatteners[var_idx](
                    x[:, var_idx, :, :]
                )  # [batch_size, model_dim*num_patches]
                z = self.linears[var_idx](z)  # [batch_size, forecast_len]
                z = self.dropouts[var_idx](z)
                outputs.append(z)  # Collect forecast per variable
            # Stack along dim=1: [batch_size, num_vars, forecast_len]
            out = torch.stack(outputs, dim=1)
        else:
            # Shared linear projection
            out = self.flatten(
                x
            )  # [batch_size, num_vars * model_dim * num_patches] if used incorrectly,
            # but here we expect flatten over last two dims
            out = self.dropout(out)
            out = self.linear(out)  # [batch_size, forecast_len]
            # Expand to [batch_size, 1, forecast_len] if needed
            out = out.unsqueeze(1) if self.num_vars == 1 else out
            # For multi-var: typically x shape → [batch_size, num_vars, model_dim, num_patches]
