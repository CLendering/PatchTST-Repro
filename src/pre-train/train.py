...
from src.utils.callback.tracking import *
from src.utils.callback.patch_mask import *
from src.utils.callback.transforms import *
from src.utils.metrics import *
from src.utils.basics import set_device
...
# Model args
parser.add_argument(
    "--n_layers", type=int, default=3, help="number of Transformer layers"
)
parser.add_argument(
    "--n_heads", type=int, default=16, help="number of Transformer heads"
)
...
# Pretrain mask
parser.add_argument(
    "--mask_ratio", type=float, default=0.4, help="masking ratio for the input"
)
# Optimization args
parser.add_argument(
    "--n_epochs_pretrain", type=int, default=10, help="number of pre-training epochs"
)
...
args.save_pretrained_model = (
    ...
    + str(args.context_points)
    + "_patch"
    + str(args.patch_len)
    + "_stride"
    + str(args.stride)
    + "_epochs-pretrain"
    + str(args.n_epochs_pretrain)
    + "_mask"
    + str(args.mask_ratio)
    + "_model"
    + str(args.pretrained_model_id)
)
args.save_path = (
    "saved_models/" + args.dset_pretrain + "/masked_patchtst/" + args.model_type + "/"
)
...
def get_model(c_in, args):
    ...
    num_patch = (
        max(args.context_points, args.patch_len) - args.patch_len
    ) // args.stride + 1
...
def find_lr():
    ...
    cbs += [
        PatchMaskCB(
            patch_len=args.patch_len, stride=args.stride, mask_ratio=args.mask_ratio
        )
    ]
